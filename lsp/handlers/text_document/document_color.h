...
std::optional<json> document_color_handler(Workspace& workspace, int /*id*/, json raw_params) {
  ...
  for (const auto& result : rgba_results) {
    ...
    while (token_idx < result.tokens.size()) {
      ...
      if (token == ":r" && result.tokens.size() > token_idx + 1) {
        red = static_cast<float>((hexToDec(result.tokens[token_idx + 1]) * 2) - 1) / 255.0f;
      } else if (token == ":g" && result.tokens.size() > token_idx + 1) {
        green = static_cast<float>((hexToDec(result.tokens[token_idx + 1]) * 2) - 1) / 255.0f;
      } else if (token == ":b" && result.tokens.size() > token_idx + 1) {
        blue = static_cast<float>((hexToDec(result.tokens[token_idx + 1]) * 2) - 1) / 255.0f;
      } else if (token == ":a" && result.tokens.size() > token_idx + 1) {
        alpha = static_cast<float>((hexToDec(result.tokens[token_idx + 1]) * 2) - 1) / 255.0f;
      }
      token_idx++;
    }
    LSPSpec::ColorInformation color_info;
    color_info.range = {{result.start_point.first, result.start_point.second},
                        {result.end_point.first, result.end_point.second}};
  ...
  for (const auto& result : static_rgba_results) {
    float red = static_cast<float>((hexToDec(result.tokens[2]) * 2) - 1) / 255.0f;
    float green = static_cast<float>((hexToDec(result.tokens[3]) * 2) - 1) / 255.0f;
    float blue = static_cast<float>((hexToDec(result.tokens[4]) * 2) - 1) / 255.0f;
    float alpha = static_cast<float>((hexToDec(result.tokens[5]) * 2) - 1) / 255.0f;
    LSPSpec::ColorInformation color_info;
    color_info.range = {{result.start_point.first, result.start_point.second},
                        {result.end_point.first, result.end_point.second}};
    color_info.color = LSPSpec::Color{red, green, blue, alpha};
