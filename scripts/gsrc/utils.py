...
def decompile_file(
    decompiler_path, decompiler_config, game_version, file_names, omit_var_casts=False
):
    decompiler_args = '{{"levels_extract": false, "process_art_groups": false, "decompile_code": true, "allowed_objects": {}}}'.format(
        file_names
    )
    if omit_var_casts:
        decompiler_args = '{{"levels_extract": false, "process_art_groups": false, "decompile_code": true, "ignore_var_name_casts": true, "allowed_objects": {}}}'.format(
            file_names
        )
    subprocess.run(
        [
