...
The `draw-inline-array-instance-tie` program builds lists per-prototype of instances. It is responsible for generating per-instance stuff, like computing distances and picking the right lod settings, and setting up DMA for per-instance colors.
...
There is a bucket per level and per mode. For example `tie-esw-l2-water` is for mode "environment mapped, scissor, water" and level 2. The "water" part means a few things:
...
### Math:
With merc/emerc, the game switched between emerc/generic-merc and there was no difference, so we knew that emerc/generic-merc had to use the same math. It looks like generic-tie/generic-merc use the same math for environment mapping.

With tie, it's not as easy. Jak 2 never uses generic-tie. It would make sense for etie to be the same as emerc/generic-merc, so you can place merc/tie stuff next to each other and it looks the same. But who knows - they could have done something like switch the texture coordinates because it was easier to write the VU program this way, and they don't have to worry about compatibility between generic-tie/etie.

### Shader:
There's an "adgif-shader" for the envmap. I'd guess there's a way for models to specify their own adgif-shader, or fall back to the default envmap.  This will need to be added to `extract_tie.cpp`.
...
I looked at the textures for "mountain", and I do see an envmap texture, but it's in the `pris` folder, and tie never seems to log it in.

All adgif shaders need to be "logged in" to be linked to a texture, so I think this means that etie always envmaps with the default texture?
...
```
these values can be added like floats, but also treated like integer memory addresses for VU1 data memory.

The `alpha` command can be modified from ???
```
