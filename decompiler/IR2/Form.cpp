...
goos::Object DefskelgroupElement::to_form_internal(const Env& env) const {
  ...
  for (const auto& e : m_info.lods) {
    auto f_dist = pretty_print::to_symbol(
        fmt::format("(meters {})", e.lod_dist->to_form(env).as_float() / 4096));
  ...
  forms.push_back(pretty_print::to_symbol(
      fmt::format(":bounds (static-spherem {} {} {} {})", m_static_info.bounds.x() / 4096,
                  m_static_info.bounds.y() / 4096, m_static_info.bounds.z() / 4096,
                  m_static_info.bounds.w() / 4096)));
  forms.push_back(pretty_print::to_symbol(
      fmt::format(":longest-edge (meters {})", m_static_info.longest_edge / 4096)));
