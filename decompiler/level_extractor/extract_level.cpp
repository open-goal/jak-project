...
std::vector<level_tools::TextureRemap> extract_tex_remap(const ObjectFileDB& db,
  ...
  std::string level_name = (bsp_rec->name.substr(bsp_rec->name.length() - 4) == "-vis")
                               ? bsp_rec->name.substr(0, bsp_rec->name.length() - 4)
                               : bsp_rec->name.substr(0, bsp_rec->name.length());
