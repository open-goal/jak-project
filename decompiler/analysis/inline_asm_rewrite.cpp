...
bool rewrite_inline_asm_instructions(Form* top_level_form,
  ...
  try {
      ...
      for (auto& entry : form->elts()) {
        ...
        if (!asmOp.valid) {
          ...
          /*lg::warn("[ASM Re-Write] - Unsupported inline assembly instruction kind - [{}]",
                   asmOp.instr.kind);*/
          f.warnings.general_error("Unsupported inline assembly instruction kind - [{}]",
          ...
          f.warnings.general_error("Inline assembly instruction marked with TODO - [{}]",
                                   asmOp.full_function_name());
        }
    ...
    lg::warn(warning);
    f.warnings.general_error(";; {}", warning);
    return false;
  }
