...
void ShadowRenderer::xgkick(u16 imm, SharedRenderState* render_state, ScopedProfilerNode& prof) {
  m_direct.render_gif((const u8*)(m_vu_data + imm), UINT32_MAX, render_state, prof);
}
...
void ShadowRenderer::render(DmaFollower& dma,
  ...
  {
    // init gs direct
    auto init_gs_dirct = dma.read_and_advance();
  }
...
void ShadowRenderer::run_mscal_vu2c(u16 imm,
  ...
  vu.vf30.add(Mask::w, vu.vf14, vu.vf14.x());   ASSERT(false);
  // nop                        |  addx.w vf31, vf15, vf15        725
  vu.vf31.add(Mask::w, vu.vf15, vu.vf15.x());
  // jr vi11                    |  subx.w vf30, vf14, vf14        726
  vu.vf30.sub(Mask::w, vu.vf14, vu.vf14.x());   ASSERT(false);
  // nop                        |  subx.w vf31, vf15, vf15        727
  vu.vf31.sub(Mask::w, vu.vf15, vu.vf15.x());
  // jr vi11                    |  addy.w vf30, vf14, vf14        728
  vu.vf30.add(Mask::w, vu.vf14, vu.vf14.y());   ASSERT(false);
  // nop                        |  addy.w vf31, vf15, vf15        729
  vu.vf31.add(Mask::w, vu.vf15, vu.vf15.y());
  // jr vi11                    |  suby.w vf30, vf14, vf14        730
  vu.vf30.sub(Mask::w, vu.vf14, vu.vf14.y());   ASSERT(false);
  // nop                        |  suby.w vf31, vf15, vf15        731
  vu.vf31.sub(Mask::w, vu.vf15, vu.vf15.y());
  // jr vi11                    |  addz.w vf30, vf14, vf14        732
  vu.vf30.add(Mask::w, vu.vf14, vu.vf14.z());   ASSERT(false);
  // nop                        |  addz.w vf31, vf15, vf15        733
  vu.vf31.add(Mask::w, vu.vf15, vu.vf15.z());
  // jr vi11                    |  subz.w vf30, vf14, vf14        734
  vu.vf30.sub(Mask::w, vu.vf14, vu.vf14.z());   ASSERT(false);
  // nop                        |  subz.w vf31, vf15, vf15        735
  vu.vf31.sub(Mask::w, vu.vf15, vu.vf15.z());
  L52:
  // nop                        |  sub.xyzw vf16, vf15, vf14      736
  vu.vf16.sub(Mask::xyzw, vu.vf15, vu.vf14);
  // waitq                      |  mul.xyzw vf16, vf16, Q         737
  vu.vf16.mul(Mask::xyzw, vu.vf16, vu.Q);   ASSERT(false);
