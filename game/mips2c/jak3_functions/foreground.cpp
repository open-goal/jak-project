...
u64 execute(void* ctxt) {
  ...
  c->lwc1(f7, 3936, at);                            // lwc1 f7, 3936(at)
  c->lwc1(f3, 3940, at);                            // lwc1 f3, 3940(at)
  c->lwc1(f6, 3944, at);                            // lwc1 f6, 3944(at)
...
// add foreground_merc::link to the link callback table for the object file.
// FWD DEC:
...
block_1:
  ...
  c->daddiu(v1, v1, 3728);                          // daddiu v1, v1, 3728
  ...
  if (((s64)c->sgpr64(s7)) == ((s64)c->sgpr64(a1))) {// beql s7, a1, L121
...
block_4:
  bc = c->sgpr64(s7) == c->sgpr64(v1);              // beq s7, v1, L155
  ...
  bc = ((s64)c->sgpr64(a3)) < 0;                    // bltz a3, L123
...
block_6:
  ...
  bc = ((s64)c->sgpr64(t2)) >= 0;                   // bgez t2, L122
  c->sq(t1, -16, a1);                               // sq t1, -16(a1)
  if (bc) {goto block_6;}                           // branch non-likely


block_7:
  bc = c->sgpr64(a0) == 0;                          // beq a0, r0, L124
...
block_12:
  ...
  c->daddiu(a3, a0, -4);                            // daddiu a3, a0, -4
  c->mov64(a1, a1);                                 // or a1, a1, r0
  bc = ((s64)c->sgpr64(a3)) < 0;                    // bltz a3, L126
  c->mov64(a2, a2);                                 // or a2, a2, r0
...
block_13:
  ...
  // nop                                            // sll r0, r0, 0
  c->sq(t0, -32, a1);                               // sq t0, -32(a1)
  bc = ((s64)c->sgpr64(t2)) >= 0;                   // bgez t2, L125
  c->sq(t1, -16, a1);                               // sq t1, -16(a1)
  if (bc) {goto block_13;}                          // branch non-likely


block_14:
  bc = c->sgpr64(a0) == 0;                          // beq a0, r0, L127
  c->lq(a3, 0, a2);                                 // lq a3, 0(a2)
  if (bc) {goto block_19;}                          // branch non-likely
...
block_19:
  ...
  bc = c->sgpr64(a1) == 0;                          // beq a1, r0, L128
  // nop                                            // sll r0, r0, 0
...
block_21:
  ...
  bc = c->sgpr64(a0) == 0;                          // beq a0, r0, L129
  // nop                                            // sll r0, r0, 0
...
block_24:
  ...
  bc = c->sgpr64(a1) == 0;                          // beq a1, r0, L131
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_26;}                          // branch non-likely

  c->addiu(a0, r0, 3776);                           // addiu a0, r0, 3776
...
block_27:
  ...
  bc = c->sgpr64(a1) == 0;                          // beq a1, r0, L133
...
block_29:
  ...
  bc = c->sgpr64(a1) != 0;                          // bne a1, r0, L134
...
block_31:
  ...
  bc = c->sgpr64(v1) == 0;                          // beq v1, r0, L135
  c->mov64(v1, s7);                                 // or v1, s7, r0
  if (bc) {goto block_33;}                          // branch non-likely
...
block_35:
  ...
  if (((s64)c->sgpr64(a0)) != ((s64)0)) {           // bnel a0, r0, L137
...
block_40:
  bc = c->sgpr64(s7) == c->sgpr64(a0);              // beq s7, a0, L138
  c->mov64(a0, s7);                                 // or a0, s7, r0
...
block_42:
  c->lbu(a0, 7, gp);                                // lbu a0, 7(gp)
  c->andi(a0, a0, 128);                             // andi a0, a0, 128
  if (((s64)c->sgpr64(a0)) == ((s64)0)) {           // beql a0, r0, L139
    c->mov64(a0, s7);                               // or a0, s7, r0
    goto block_45;
  }
...
block_45:
  bc = c->sgpr64(s7) == c->sgpr64(a0);              // beq s7, a0, L140
  c->mov64(a0, s7);                                 // or a0, s7, r0
...
block_61:
  ...
  bc = ((s64)c->sgpr64(a2)) < 0;                    // bltz a2, L148
  c->mov64(a1, a1);                                 // or a1, a1, r0
...
block_62:
  ...
  bc = ((s64)c->sgpr64(t1)) >= 0;                   // bgez t1, L147
  c->sq(t0, -16, a0);                               // sq t0, -16(a0)
  if (bc) {goto block_62;}                          // branch non-likely


block_63:
  bc = c->sgpr64(v1) == 0;                          // beq v1, r0, L149
...
block_68:
  ...
  if (((s64)c->sgpr64(a0)) == ((s64)0)) {           // beql a0, r0, L150
    c->mov64(a0, s7);                               // or a0, s7, r0
    goto block_71;
  }
...
block_71:
  bc = c->sgpr64(s7) == c->sgpr64(a0);              // beq s7, a0, L154
  c->mov64(a0, s7);                                 // or a0, s7, r0
  ...
  c->daddiu(a3, a0, -4);                            // daddiu a3, a0, -4
  c->mov64(a1, a1);                                 // or a1, a1, r0
  bc = ((s64)c->sgpr64(a3)) < 0;                    // bltz a3, L152
  c->mov64(a2, a2);                                 // or a2, a2, r0
...
block_73:
  ...
  // nop                                            // sll r0, r0, 0
  c->sq(t0, -32, a1);                               // sq t0, -32(a1)
  bc = ((s64)c->sgpr64(t2)) >= 0;                   // bgez t2, L151
  c->sq(t1, -16, a1);                               // sq t1, -16(a1)
  if (bc) {goto block_73;}                          // branch non-likely


block_74:
  bc = c->sgpr64(a0) == 0;                          // beq a0, r0, L153
  c->lq(a3, 0, a2);                                 // lq a3, 0(a2)
...
block_82:
  ...
  bc = c->sgpr64(v1) != 0;                          // bne v1, r0, L120
  // nop                                            // sll r0, r0, 0
...
// add foreground_generic_merc::link to the link callback table for the object file.
// FWD DEC:
//--------------------------MIPS2C---------------------
// clang-format off
#include "game/mips2c/mips2c_private.h"
...
block_8:
  ...
  c->daddiu(t0, t0, 172);                           // daddiu t0, t0, 172
  ...
  c->addiu(t0, r0, 3824);                           // addiu t0, r0, 3824
  get_fake_spad_addr2(t2, cache.fake_scratchpad_data, 0, c);// lui t2, 28672
...
void link() {
  cache.bone_calculation_list = intern_from_c(-1, 0, "*bone-calculation-list*").c();
  cache.display = intern_from_c(-1, 0, "*display*").c();
  cache.fake_scratchpad_data = intern_from_c(-1, 0, "*fake-scratchpad-data*").c();
  cache.foreground = intern_from_c(-1, 0, "*foreground*").c();
  cache.foreground_generic_merc = intern_from_c(-1, 0, "foreground-generic-merc").c();
  gLinkedFunctionTable.reg("foreground-draw-hud", execute, 256);
}

} // namespace foreground_draw_hud
} // namespace Mips2C
// add foreground_draw_hud::link to the link callback table for the object file.
// FWD DEC:
