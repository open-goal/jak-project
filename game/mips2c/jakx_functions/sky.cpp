...
u64 execute(void* ctxt) {
  ...
  c->lqc2(vf31, 1536, gp);                          // lqc2 vf31, 1536(gp)
  c->lqc2(vf30, 1552, gp);                          // lqc2 vf30, 1552(gp)
  c->lqc2(vf29, 1568, gp);                          // lqc2 vf29, 1568(gp)
...
block_1:
  ...
  // nop                                            // sll r0, r0, 0
  bc = ((s64)c->sgpr64(a2)) >= 0;                   // bgez a2, L34
  // nop                                            // sll r0, r0, 0
...
block_3:
  bc = ((s64)c->sgpr64(a3)) >= 0;                   // bgez a3, L35
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_5;}                           // branch non-likely
...
block_5:
  bc = ((s64)c->sgpr64(t0)) >= 0;                   // bgez t0, L36
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_7;}                           // branch non-likely
...
block_7:
  bc = ((s64)c->sgpr64(t1)) >= 0;                   // bgez t1, L37
  // nop                                            // sll r0, r0, 0
...
block_9:
  c->sqc2(vf15, 1920, v1);                          // sqc2 vf15, 1920(v1)
  // nop                                            // sll r0, r0, 0
  c->sqc2(vf16, 1936, v1);                          // sqc2 vf16, 1936(v1)
  // nop                                            // sll r0, r0, 0
  c->sqc2(vf17, 1952, v1);                          // sqc2 vf17, 1952(v1)
  // nop                                            // sll r0, r0, 0
  c->sqc2(vf18, 1968, v1);                          // sqc2 vf18, 1968(v1)
...
block_12:
  ...
  c->daddiu(v1, v1, 1920);                          // daddiu v1, v1, 1920
...
void link() {
  ...
  gLinkedFunctionTable.reg("(method 32 sky-work)", execute, 256);
}
...
u64 execute(void* ctxt) {
  ...
  c->load_symbol2(v1, cache.z_vector);              // lw v1, *z-vector*(s7)
  c->daddiu(a0, s4, 32);                            // daddiu a0, s4, 32
...
block_1:
  ...
  c->daddiu(a0, a0, 528);                           // daddiu a0, a0, 528
  c->daddu(a0, a0, gp);                             // daddu a0, a0, gp
  c->cvtws(f2, f1);                                 // cvt.w.s f2, f1
  c->swc1(f2, 0, a0);                               // swc1 f2, 0(a0)
  c->cvtws(f2, f1);                                 // cvt.w.s f2, f1
  c->swc1(f2, 4, a0);                               // swc1 f2, 4(a0)
...
block_2:
  ...
  bc = c->sgpr64(a0) != 0;                          // bne a0, r0, L42
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
...
void link() {
  cache.z_vector = intern_from_c(-1, 0, "*z-vector*").c();
  cache.vector_normalize = intern_from_c(-1, 0, "vector-normalize!").c();
  gLinkedFunctionTable.reg("(method 31 sky-work)", execute, 256);
}
...
struct Cache {
  // TODO FIX - void* adgif_shader<_texture_simple; // adgif-shader<-texture-simple!
...
u64 execute(void* ctxt) {
  ...
  c->load_symbol2(t9, cache.lookup_texture_by_id_fast);// lw t9, lookup-texture-by-id-fast(s7)
  c->lui(v1, 128);                                  // lui v1, 128
  c->ori(a0, v1, 2048);                             // ori a0, v1, 2048
  call_addr = c->gprs[t9].du32[0];                  // function call:
  c->sll(v0, ra, 0);                                // sll v0, ra, 0
  c->jalr(call_addr);                               // jalr ra, t9
  c->sw(v0, 20, sp);                                // sw v0, 20(sp)
  c->lwu(v1, 16, sp);                               // lwu v1, 16(sp)
  if (((s64)c->sgpr64(s7)) == ((s64)c->sgpr64(v1))) {// beql s7, v1, L45
...
block_3:
  bc = c->sgpr64(s7) == c->sgpr64(v1);              // beq s7, v1, L48
  c->mov64(v1, s7);                                 // or v1, s7, r0
  ...
  c->lqc2(vf2, 784, gp);                            // lqc2 vf2, 784(gp)
  ...
  c->lqc2(vf5, 800, gp);                            // lqc2 vf5, 800(gp)
  ...
  c->lqc2(vf9, 512, gp);                            // lqc2 vf9, 512(gp)
  c->vwaitq();                                      // vwaitq
  ...
  c->sqc2(vf7, 1600, gp);                           // sqc2 vf7, 1600(gp)
  c->sqc2(vf10, 1616, gp);                          // sqc2 vf10, 1616(gp)
  c->lw(v1, 1612, gp);                              // lw v1, 1612(gp)
...
block_6:
  if (((s64)c->sgpr64(s7)) == ((s64)c->sgpr64(v1))) {// beql s7, v1, L47
    c->mov64(v1, v1);                               // or v1, v1, r0
    goto block_15;
  }
  
block_8:
  c->lw(v1, 1600, gp);                              // lw v1, 1600(gp)
  c->lw(a0, 556, s5);                               // lw a0, 556(s5)
  c->dsll(a0, a0, 4);                               // dsll a0, a0, 4
  c->slt(v1, v1, a0);                               // slt v1, v1, a0
  c->daddiu(a0, s7, 4);                             // daddiu a0, s7, 4
  c->movz(a0, s7, v1);                              // movz a0, s7, v1
  if (((s64)c->sgpr64(s7)) == ((s64)c->sgpr64(a0))) {// beql s7, a0, L47
    c->mov64(v1, a0);                               // or v1, a0, r0
    goto block_15;
  }
  
block_10:
  c->lw(v1, 1604, gp);                              // lw v1, 1604(gp)
  c->lw(a0, 560, s5);                               // lw a0, 560(s5)
  c->dsll(a0, a0, 4);                               // dsll a0, a0, 4
  c->slt(v1, v1, a0);                               // slt v1, v1, a0
  c->daddiu(a0, s7, 4);                             // daddiu a0, s7, 4
  c->movz(a0, s7, v1);                              // movz a0, s7, v1
...
block_15:
  ...
  c->lwu(v1, 1916, gp);                             // lwu v1, 1916(gp)
  ...
  c->sqc2(vf11, 128, t0);                           // sqc2 vf11, 128(t0)
  c->sqc2(vf3, 144, t0);                            // sqc2 vf3, 144(t0)
  c->sqc2(vf2, 160, t0);                            // sqc2 vf2, 160(t0)
  c->sqc2(vf1, 176, t0);                            // sqc2 vf1, 176(t0)
  c->sqc2(vf5, 192, t0);                            // sqc2 vf5, 192(t0)
  c->sqc2(vf4, 208, t0);                            // sqc2 vf4, 208(t0)
  c->sqc2(vf8, 224, t0);                            // sqc2 vf8, 224(t0)
  c->sqc2(vf11, 240, t0);                           // sqc2 vf11, 240(t0)
  c->sqc2(vf6, 256, t0);                            // sqc2 vf6, 256(t0)
  c->sqc2(vf2, 272, t0);                            // sqc2 vf2, 272(t0)
  c->sqc2(vf7, 288, t0);                            // sqc2 vf7, 288(t0)
  c->sqc2(vf5, 304, t0);                            // sqc2 vf5, 304(t0)
  c->sqc2(vf10, 320, t0);                           // sqc2 vf10, 320(t0)
  c->sqc2(vf8, 448, t0);                            // sqc2 vf8, 448(t0)
  c->sqc2(vf11, 464, t0);                           // sqc2 vf11, 464(t0)
  c->sqc2(vf9, 480, t0);                            // sqc2 vf9, 480(t0)
  c->sqc2(vf2, 496, t0);                            // sqc2 vf2, 496(t0)
  c->sqc2(vf17, 512, t0);                           // sqc2 vf17, 512(t0)
  c->sqc2(vf5, 528, t0);                            // sqc2 vf5, 528(t0)
  c->sqc2(vf18, 544, t0);                           // sqc2 vf18, 544(t0)
  c->lwu(v1, 1916, gp);                             // lwu v1, 1916(gp)
  ...
  // TODO - fix c->load_symbol2(t9, cache.adgif_shader<_texture_simple);// lw t9, adgif-shader<-texture-simple!(s7)
  c->mov64(a0, s4);                                 // or a0, s4, r0
  ...
  c->lwu(a0, 1916, gp);                             // lwu a0, 1916(gp)
...
void link() {
  // TODO FIX - cache.adgif_shader<_texture_simple = intern_from_c(-1, 0, "adgif-shader<-texture-simple!").c();
  cache.lookup_texture_by_id_fast = intern_from_c(-1, 0, "lookup-texture-by-id-fast").c();
  gLinkedFunctionTable.reg("(method 30 sky-work)", execute, 256);
}
...
u64 execute(void* ctxt) {
  ...
  c->sw(v0, 16, sp);                                // sw v0, 16(sp)
  c->lwu(v1, 16, sp);                               // lwu v1, 16(sp)
  bc = c->sgpr64(s7) == c->sgpr64(v1);              // beq s7, v1, L52
  c->mov64(v1, s7);                                 // or v1, s7, r0
  ...
  c->vftoi4(DEST::xyzw, vf4, vf4);                  // vftoi4.xyzw vf4, vf4
  c->sqc2(vf1, 1600, gp);                           // sqc2 vf1, 1600(gp)
  c->sqc2(vf4, 1616, gp);                           // sqc2 vf4, 1616(gp)
  c->lw(v1, 1612, gp);                              // lw v1, 1612(gp)
  c->mtc1(f0, v1);                                  // mtc1 f0, v1
  c->mtc1(f1, r0);                                  // mtc1 f1, r0
  cop1_bc = c->fprs[f0] < c->fprs[f1];              // c.lt.s f0, f1
...
block_5:
  ...
  c->daddiu(a0, s7, 4);                             // daddiu a0, s7, 4
  c->movz(a0, s7, v1);                              // movz a0, s7, v1
  if (((s64)c->sgpr64(s7)) == ((s64)c->sgpr64(a0))) {// beql s7, a0, L51
    c->mov64(v1, a0);                               // or v1, a0, r0
    goto block_12;
  }
...
block_12:
  ...
  c->sqc2(vf6, 256, t0);                            // sqc2 vf6, 256(t0)
  c->sqc2(vf2, 272, t0);                            // sqc2 vf2, 272(t0)
  c->sqc2(vf1, 288, t0);                            // sqc2 vf1, 288(t0)
  c->sqc2(vf5, 304, t0);                            // sqc2 vf5, 304(t0)
  c->sqc2(vf4, 320, t0);                            // sqc2 vf4, 320(t0)
  c->lwu(v1, 1916, gp);                             // lwu v1, 1916(gp)
  c->lwu(v1, 4, v1);                                // lwu v1, 4(v1)
...
void link() {
  // TODO - fix cache.adgif_shader<_texture_simple = intern_from_c(-1, 0, "adgif-shader<-texture-simple!").c();
  cache.lookup_texture_by_id_fast = intern_from_c(-1, 0, "lookup-texture-by-id-fast").c();
  gLinkedFunctionTable.reg("(method 29 sky-work)", execute, 256);
}
...
u64 execute(void* ctxt) {
  ...
  c->lwu(a2, 176, a1);                              // lwu a2, 176(a1)
  c->daddiu(v1, a0, 1472);                          // daddiu v1, a0, 1472
  ...
  c->lqc2(vf13, 1024, a0);                          // lqc2 vf13, 1024(a0)
...
void link() {
  gLinkedFunctionTable.reg("(method 17 sky-work)", execute, 256);
}
...
void link() {
  ...
  gLinkedFunctionTable.reg("render-sky-tri", execute, 256);
}
...
void link() {
  cache.fake_scratchpad_data = intern_from_c(-1, 0, "*fake-scratchpad-data*").c();
  cache.draw_large_polygon = intern_from_c(-1, 0, "draw-large-polygon").c();
  gLinkedFunctionTable.reg("render-sky-quad", execute, 256);
}
...
u64 execute(void* ctxt) {
  ...
  c->lqc2(vf3, -16, a3);                            // lqc2 vf3, -16(a3)
  if (bc) {goto block_9;}                           // branch non-likely
...
block_1:
  ...
  c->daddiu(t3, t3, 48);                            // daddiu t3, t3, 48
  c->lqc2(vf4, 0, a3);                              // lqc2 vf4, 0(a3)
  c->negs(f2, f2);                                  // neg.s f2, f2
  // nop                                            // sll r0, r0, 0
  c->lqc2(vf5, 16, a3);                             // lqc2 vf5, 16(a3)
  ...
  bc = c->sgpr64(t2) == 0;                          // beq t2, r0, L110
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_17;}                          // branch non-likely
...
block_3:
  ...
  cop1_bc = c->fprs[f1] < c->fprs[f0];              // c.lt.s f1, f0
  c->daddiu(a3, a3, 48);                            // daddiu a3, a3, 48
  bc = !cop1_bc;                                    // bc1f L105
  c->lqc2(vf3, -16, a3);                            // lqc2 vf3, -16(a3)
  if (bc) {goto block_8;}                           // branch non-likely

  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L102
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
...
block_6:
  ...
  c->daddiu(t7, s7, 4);                             // daddiu t7, s7, 4
  ...
  // nop                                            // sll r0, r0, 0
  c->sqc2(vf7, -48, t0);                            // sqc2 vf7, -48(t0)
  // nop                                            // sll r0, r0, 0
  c->sqc2(vf8, -32, t0);                            // sqc2 vf8, -32(t0)
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L107
  c->sqc2(vf9, -16, t0);                            // sqc2 vf9, -16(t0)
...
block_9:
  ...
  cop1_bc = c->fprs[f3] < c->fprs[f2];              // c.lt.s f3, f2
  c->daddiu(a3, a3, 48);                            // daddiu a3, a3, 48
  bc = cop1_bc;                                     // bc1t L108
  c->lqc2(vf6, -16, a3);                            // lqc2 vf6, -16(a3)
  if (bc) {goto block_14;}                          // branch non-likely

  c->sqc2(vf1, 0, t0);                              // sqc2 vf1, 0(t0)
  c->daddiu(t0, t0, 48);                            // daddiu t0, t0, 48
  c->sqc2(vf2, -32, t0);                            // sqc2 vf2, -32(t0)
  c->daddiu(t1, t1, 1);                             // daddiu t1, t1, 1
  bc = c->sgpr64(t2) == 0;                          // beq t2, r0, L110
  c->sqc2(vf3, -16, t0);                            // sqc2 vf3, -16(t0)
  if (bc) {goto block_17;}                          // branch non-likely
...
block_11:
  ...
  cop1_bc = c->fprs[f1] < c->fprs[f0];              // c.lt.s f1, f0
  c->daddiu(a3, a3, 48);                            // daddiu a3, a3, 48
  bc = cop1_bc;                                     // bc1t L109
  c->lqc2(vf3, -16, a3);                            // lqc2 vf3, -16(a3)
  if (bc) {goto block_16;}                          // branch non-likely

  c->sqc2(vf4, 0, t0);                              // sqc2 vf4, 0(t0)
  c->daddiu(t0, t0, 48);                            // daddiu t0, t0, 48
  c->sqc2(vf5, -32, t0);                            // sqc2 vf5, -32(t0)
  c->daddiu(t1, t1, 1);                             // daddiu t1, t1, 1
  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L106
  c->sqc2(vf6, -16, t0);                            // sqc2 vf6, -16(t0)
  if (bc) {goto block_9;}                           // branch non-likely
...
block_14:
  ...
  c->vsub(DEST::xyzw, vf9, vf3, vf6);               // vsub.xyzw vf9, vf3, vf6
  c->divs(f6, f8, f5);                              // div.s f6, f8, f5
  c->daddiu(t0, t0, 96);                            // daddiu t0, t0, 96
  c->mfc1(v1, f6);                                  // mfc1 v1, f6
  c->mov128_vf_gpr(vf10, v1);                       // qmtc2.i vf10, v1
  c->sqc2(vf1, -96, t0);                            // sqc2 vf1, -96(t0)
  c->vmula_bc(DEST::xyzw, BC::w, vf4, vf0);         // vmulaw.xyzw acc, vf4, vf0
  c->sqc2(vf2, -80, t0);                            // sqc2 vf2, -80(t0)
  c->vmadd_bc(DEST::xyzw, BC::x, vf7, vf7, vf10);   // vmaddx.xyzw vf7, vf7, vf10
  c->sqc2(vf3, -64, t0);                            // sqc2 vf3, -64(t0)
  c->vmula_bc(DEST::xyzw, BC::w, vf5, vf0);         // vmulaw.xyzw acc, vf5, vf0
  ...
  c->sqc2(vf8, -32, t0);                            // sqc2 vf8, -32(t0)
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L103
  c->sqc2(vf9, -16, t0);                            // sqc2 vf9, -16(t0)
  if (bc) {goto block_3;}                           // branch non-likely
...
block_16:
  ...
  c->mfc1(v1, f6);                                  // mfc1 v1, f6
  c->mov128_vf_gpr(vf10, v1);                       // qmtc2.i vf10, v1
  c->sqc2(vf4, -96, t0);                            // sqc2 vf4, -96(t0)
  c->vmula_bc(DEST::xyzw, BC::w, vf1, vf0);         // vmulaw.xyzw acc, vf1, vf0
  c->sqc2(vf5, -80, t0);                            // sqc2 vf5, -80(t0)
  c->vmadd_bc(DEST::xyzw, BC::x, vf7, vf7, vf10);   // vmaddx.xyzw vf7, vf7, vf10
  c->sqc2(vf6, -64, t0);                            // sqc2 vf6, -64(t0)
  c->vmula_bc(DEST::xyzw, BC::w, vf2, vf0);         // vmulaw.xyzw acc, vf2, vf0
  c->daddiu(t1, t1, 2);                             // daddiu t1, t1, 2
  c->vmadd_bc(DEST::xyzw, BC::x, vf8, vf8, vf10);   // vmaddx.xyzw vf8, vf8, vf10
  c->daddiu(t7, s7, 4);                             // daddiu t7, s7, 4
...
void link() {
  gLinkedFunctionTable.reg("clip-polygon-against-negative-hyperplane", execute, 256);
}
...
u64 execute(void* ctxt) {
  ...
  c->daddiu(a3, a3, 48);                            // daddiu a3, a3, 48
  cop1_bc = c->fprs[f0] < c->fprs[f1];              // c.lt.s f0, f1
  c->lqc2(vf2, -32, a3);                            // lqc2 vf2, -32(a3)
  bc = !cop1_bc;                                    // bc1f L116
  c->lqc2(vf3, -16, a3);                            // lqc2 vf3, -16(a3)
  if (bc) {goto block_9;}                           // branch non-likely
...
block_1:
  ...
  c->lqc2(vf5, -32, a3);                            // lqc2 vf5, -32(a3)
  bc = !cop1_bc;                                    // bc1f L114
  c->lqc2(vf6, -16, a3);                            // lqc2 vf6, -16(a3)
  if (bc) {goto block_6;}                           // branch non-likely

  bc = c->sgpr64(t2) == 0;                          // beq t2, r0, L120
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_17;}                          // branch non-likely
...
block_3:
  ...
  cop1_bc = c->fprs[f0] < c->fprs[f1];              // c.lt.s f0, f1
  c->lqc2(vf2, -32, a3);                            // lqc2 vf2, -32(a3)
  bc = !cop1_bc;                                    // bc1f L115
  c->lqc2(vf3, -16, a3);                            // lqc2 vf3, -16(a3)
  if (bc) {goto block_8;}                           // branch non-likely

  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L112
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
...
block_6:
  ...
  c->sqc2(vf8, -32, t0);                            // sqc2 vf8, -32(t0)
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L117
  c->sqc2(vf9, -16, t0);                            // sqc2 vf9, -16(t0)
  if (bc) {goto block_11;}                          // branch non-likely
...
block_11:
  ...
  c->sqc2(vf5, -32, t0);                            // sqc2 vf5, -32(t0)
  c->daddiu(t1, t1, 1);                             // daddiu t1, t1, 1
  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L116
  c->sqc2(vf6, -16, t0);                            // sqc2 vf6, -16(t0)
  if (bc) {goto block_9;}                           // branch non-likely
...
block_14:
  ...
  c->sqc2(vf8, -32, t0);                            // sqc2 vf8, -32(t0)
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(t2) != 0;                          // bne t2, r0, L113
  c->sqc2(vf9, -16, t0);                            // sqc2 vf9, -16(t0)
  if (bc) {goto block_3;}                           // branch non-likely
...
void link() {
  gLinkedFunctionTable.reg("clip-polygon-against-positive-hyperplane", execute, 256);
}
...
void link() {
  gLinkedFunctionTable.reg("set-tex-offset", execute, 256);
}
