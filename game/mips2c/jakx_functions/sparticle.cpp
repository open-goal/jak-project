...
block_1:
  ...
  bc = c->sgpr64(s0) != 0;                          // bne s0, r0, L71
  // nop                                            // sll r0, r0, 0
...
block_14:
  ...
  c->dsubu(v1, v1, s0);                             // dsubu v1, v1, s0
...
block_19:
  ...
  c->mov64(s0, s0);                                 // or s0, s0, r0
  
block_20:
  c->load_symbol2(t9, cache.quaternion);            // lw t9, quaternion*!(s7)
  c->lwu(a0, 32, sp);                               // lwu a0, 32(sp)
  c->lwu(a1, 32, sp);                               // lwu a1, 32(sp)
  c->daddiu(a2, s5, 80);                            // daddiu a2, s5, 80
  call_addr = c->gprs[t9].du32[0];                  // function call:
  c->sll(v0, ra, 0);                                // sll v0, ra, 0
  c->jalr(call_addr);                               // jalr ra, t9
  c->daddiu(s0, s0, -10);                           // daddiu s0, s0, -10
  // nop                                            // sll r0, r0, 0
  bc = ((s64)c->sgpr64(s0)) > 0;                    // bgtz s0, L77
  // nop                                            // sll r0, r0, 0
...
block_1:
  ...
  c->mov128_gpr_vf(v1, vf9);                        // qmfc2.i v1, vf9
  c->andi(s0, v1, 255);                             // andi s0, v1, 255
  bc = c->sgpr64(s0) != 0;                          // bne s0, r0, L88
  // nop                                            // sll r0, r0, 0
  ...
  // nop                                            // sll r0, r0, 0
  c->lui(a0, 1);                                    // lui a0, 1
  c->ori(a0, a0, 128);                              // ori a0, a0, 128
  c->and_(v1, v1, a0);                              // and v1, v1, a0
  bc = c->sgpr64(v1) == 0;                          // beq v1, r0, L86
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_5;}                           // branch non-likely
...
block_9:
  c->lw(v1, 100, s5);                               // lw v1, 100(s5)
  c->addiu(a0, r0, -1);                             // addiu a0, r0, -1
  bc = c->sgpr64(v1) == c->sgpr64(a0);              // beq v1, a0, L89
  c->dsubu(a0, v1, s0);                             // dsubu a0, v1, s0
  if (bc) {goto block_12;}                          // branch non-likely
...
block_14:
  ...
  bc = c->sgpr64(t9) == 0;                          // beq t9, r0, L91
  // nop                                            // sll r0, r0, 0
...
block_16:
  c->lw(a1, 120, s5);                               // lw a1, 120(s5)
  c->lw(v1, 116, s5);                               // lw v1, 116(s5)
  bc = c->sgpr64(a1) == 0;                          // beq a1, r0, L92
  c->dsubu(a0, v1, s0);                             // dsubu a0, v1, s0
  if (bc) {goto block_19;}                          // branch non-likely

  c->daddiu(v1, a0, -1);                            // daddiu v1, a0, -1
  bc = ((s64)c->sgpr64(v1)) >= 0;                   // bgez v1, L92
...
block_21:
  ...
  bc = c->sgpr64(v1) == 0;                          // beq v1, r0, L94
...
block_23:
  ...
  bc = c->sgpr64(t4) == 0;                          // beq t4, r0, L97
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_31;}                          // branch non-likely
...
block_26:
  ...
  bc = ((s64)c->sgpr64(t4)) <= 0;                   // blez t4, L97
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_31;}                          // branch non-likely
...
void link() {
  ...
  cache.sp_free_particle = intern_from_c(-1, 0, "sp-free-particle").c();
  cache.sp_orbiter = intern_from_c(-1, 0, "sp-orbiter").c();
  cache.sp_relaunch_particle_2d = intern_from_c(-1, 0, "sp-relaunch-particle-2d").c();
  gLinkedFunctionTable.reg("sp-process-block-2d", execute, 512);
}
