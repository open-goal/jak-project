...
void link() {
  ...
  gLinkedFunctionTable.reg("(method 9 collide-puss-work)", execute, 512);
}
...
void link() {
  ...
  gLinkedFunctionTable.reg("(method 17 collide-cache)", execute, 256);
}
...
block_1:
  bc = c->sgpr64(s3) == 0;                          // beq s3, r0, L54
...
block_2:
  ...
  c->dsll(v1, v1, 16);                              // dsll v1, v1, 16
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(v1) != 0;                          // bne v1, r0, L52
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
  ...
  c->jalr(call_addr);                               // jalr ra, t9
  c->mov64(v1, v0);                                 // or v1, v0, r0
  bc = c->sgpr64(s3) != 0;                          // bne s3, r0, L53
  c->daddiu(s4, s4, 80);                            // daddiu s4, s4, 80
  if (bc) {goto block_2;}                           // branch non-likely
...
void link() {
  gLinkedFunctionTable.reg("(method 11 collide-shape-prim-group)", execute, 512);
}
...
void link() {
  cache.format = intern_from_c(-1, 0, "format").c();
  gLinkedFunctionTable.reg("(method 11 collide-shape-prim-sphere)", execute, 256);
}
...
u64 execute(void* ctxt) {
  ...
  c->lw(v1, 108, gp);                               // lw v1, 108(gp)
  c->lwu(a0, 8, gp);                                // lwu a0, 8(gp)
  bc = c->sgpr64(s2) == c->sgpr64(s7);              // beq s2, s7, L63
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_14;}                          // branch non-likely

  c->lwu(a1, 12, gp);                               // lwu a1, 12(gp)
  c->dsll(a2, a0, 1);                               // dsll a2, a0, 1
  bc = c->sgpr64(a0) == c->sgpr64(a1);              // beq a0, a1, L61
  ...
  bc = ((s64)c->sgpr64(a0)) < 0;                    // bltz a0, L62
  c->daddiu(a2, s2, 28);                            // daddiu a2, s2, 28
  if (bc) {goto block_12;}                          // branch non-likely

  c->mov64(a0, v1);                                 // or a0, v1, r0
  c->lwu(a3, 4, s2);                                // lwu a3, 4(s2)
  c->lw(t2, 124, gp);                               // lw t2, 124(gp)
  ...
  c->lwu(t4, 16, gp);                               // lwu t4, 16(gp)
  c->vsub(DEST::zw, vf1, vf0, vf0);                 // vsub.zw vf1, vf0, vf0
  // nop                                            // sll r0, r0, 0
  
block_4:
  bc = c->sgpr64(a3) == 0;                          // beq a3, r0, L60
  ...
  bc = c->sgpr64(t9) != 0;                          // bne t9, r0, L59
  c->daddiu(a2, a2, 8);                             // daddiu a2, a2, 8
  if (bc) {goto block_4;}                           // branch non-likely

  bc = c->sgpr64(v1) == c->sgpr64(a1);              // beq v1, a1, L62
...
block_9:
  ...
  c->lwu(t0, 8, gp);                                // lwu t0, 8(gp)
  bc = c->sgpr64(a3) == 0;                          // beq a3, r0, L63
...
block_12:
  ...
  bc = c->sgpr64(a0) != c->sgpr64(v1);              // bne a0, v1, L63
  c->mov64(v1, s7);                                 // or v1, s7, r0
  if (bc) {goto block_14;}                          // branch non-likely
...
void link() {
  ...
  gLinkedFunctionTable.reg("(method 11 collide-shape-prim-mesh)", execute, 256);
}
...
block_1:
  bc = c->sgpr64(s4) == 0;                          // beq s4, r0, L83
...
block_2:
  ...
  c->dsll(v1, v1, 16);                              // dsll v1, v1, 16
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(v1) != 0;                          // bne v1, r0, L81
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
  ...
  c->jalr(call_addr);                               // jalr ra, t9
  c->mov64(v1, v0);                                 // or v1, v0, r0
  bc = c->sgpr64(s4) != 0;                          // bne s4, r0, L82
  c->daddiu(s5, s5, 80);                            // daddiu s5, s5, 80
  if (bc) {goto block_2;}                           // branch non-likely
...
void link() {
  gLinkedFunctionTable.reg("(method 10 collide-shape-prim-group)", execute, 512);
}
...
u64 execute(void* ctxt) {
  ...
  c->dsll(t2, a3, 1);                               // dsll t2, a3, 1
  c->lq(a2, 28, a0);                                // lq a2, 28(a0)
  bc = c->sgpr64(a3) == c->sgpr64(t1);              // beq a3, t1, L85
  c->daddu(t1, t2, a3);                             // daddu t1, t2, a3
  if (bc) {goto block_2;}                           // branch non-likely
...
void link() {
  cache.format = intern_from_c(-1, 0, "format").c();
  gLinkedFunctionTable.reg("(method 10 collide-shape-prim-sphere)", execute, 256);
}
...
struct Cache {
  void* cheat_mode; // *cheat-mode*
  void* display_capture_mode; // *display-capture-mode*
  void* fake_scratchpad_data; // *fake-scratchpad-data*
  void* debug; // debug
  void* format; // format
  void* print_exceeded_max_cache_tris; // print-exceeded-max-cache-tris
...
u64 execute(void* ctxt) {
  ...
  c->lw(v1, 108, gp);                               // lw v1, 108(gp)
  c->lwu(a0, 8, gp);                                // lwu a0, 8(gp)
  bc = c->sgpr64(s3) == c->sgpr64(s7);              // beq s3, s7, L93
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_18;}                          // branch non-likely

  c->lwu(a1, 12, gp);                               // lwu a1, 12(gp)
  c->dsll(a2, a0, 1);                               // dsll a2, a0, 1
  bc = c->sgpr64(a0) == c->sgpr64(a1);              // beq a0, a1, L90
  c->daddu(a0, a2, a0);                             // daddu a0, a2, a0
  if (bc) {goto block_11;}                          // branch non-likely
  ...
  // nop                                            // sll r0, r0, 0
  // nop                                            // sll r0, r0, 0
  c->lwu(a1, 8, gp);                                // lwu a1, 8(gp)
  c->lwu(a2, 4, gp);                                // lwu a2, 4(gp)
  c->lwu(v1, 0, gp);                                // lwu v1, 0(gp)
  ...
  bc = ((s64)c->sgpr64(a0)) < 0;                    // bltz a0, L91
  c->daddiu(a3, s3, 28);                            // daddiu a3, s3, 28
  if (bc) {goto block_12;}                          // branch non-likely

  c->mov64(a0, v1);                                 // or a0, v1, r0
  c->lwu(t0, 4, s3);                                // lwu t0, 4(s3)
  c->lw(t3, 124, gp);                               // lw t3, 124(gp)
...
block_4:
  ...
  c->lq(t6, 0, t6);                                 // lq t6, 0(t6)
  bc = c->sgpr64(ra) != 0;                          // bne ra, r0, L88
  c->daddiu(a3, a3, 8);                             // daddiu a3, a3, 8
  if (bc) {goto block_4;}                           // branch non-likely

  bc = c->sgpr64(v1) == c->sgpr64(a2);              // beq v1, a2, L91
...
block_9:
  c->dsubu(a3, v1, a0);                             // dsubu a3, v1, a0
  c->lwu(t0, 8, gp);                                // lwu t0, 8(gp)
  bc = c->sgpr64(a3) == 0;                          // beq a3, r0, L93
  c->lq(a1, 12, s5);                                // lq a1, 12(s5)
  if (bc) {goto block_18;}                          // branch non-likely
...
block_12:
  c->load_symbol2(v1, cache.cheat_mode);            // lw v1, *cheat-mode*(s7)
  c->load_symbol_addr(a0, cache.debug);             // daddiu a0, s7, debug
  c->dsubu(v1, v1, a0);                             // dsubu v1, v1, a0
  c->daddiu(a0, s7, 4);                             // daddiu a0, s7, 4
  c->movn(a0, s7, v1);                              // movn a0, s7, v1
  if (((s64)c->sgpr64(s7)) == ((s64)c->sgpr64(a0))) {// beql s7, a0, L92
...
void link() {
  cache.cheat_mode = intern_from_c(-1, 0, "*cheat-mode*").c();
  cache.display_capture_mode = intern_from_c(-1, 0, "*display-capture-mode*").c();
  cache.fake_scratchpad_data = intern_from_c(-1, 0, "*fake-scratchpad-data*").c();
  cache.debug = intern_from_c(-1, 0, "debug").c();
  cache.format = intern_from_c(-1, 0, "format").c();
  cache.print_exceeded_max_cache_tris = intern_from_c(-1, 0, "print-exceeded-max-cache-tris").c();
