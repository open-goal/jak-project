...
block_6:
  bc = ((s64)c->sgpr64(v1)) < 0;                    // bltz v1, L27
  // nop                                            // sll r0, r0, 0
...
block_8:
  ...
  bc = c->sgpr64(a3) == 0;                          // beq a3, r0, L33
  ...
  bc = c->sgpr64(a2) == 0;                          // beq a2, r0, L31
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_19;}                          // branch non-likely

  bc = c->sgpr64(a1) == 0;                          // beq a1, r0, L32
  // nop                                            // sll r0, r0, 0
  ...
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(a2) == 0;                          // beq a2, r0, L28
  // nop                                            // sll r0, r0, 0
...
block_14:
  bc = c->sgpr64(a1) == 0;                          // beq a1, r0, L29
  // nop                                            // sll r0, r0, 0
...
block_16:
  bc = c->sgpr64(v1) == 0;                          // beq v1, r0, L33
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_21;}                          // branch non-likely
...
// add add_light_sphere_to_light_group::link to the link callback table for the object file.
// FWD DEC:
...
u64 execute(void* ctxt) {
  ...
  bc = c->sgpr64(a1) != 0;                          // bne a1, r0, L35
  // nop                                            // sll r0, r0, 0
...
// add light_hash_get_bucket_index::link to the link callback table for the object file.
// FWD DEC:
...
void link() {
  cache.light_hash_work = intern_from_c(-1, 0, "*light-hash-work*").c();
  gLinkedFunctionTable.reg("light-hash-add-items", execute, 64);
}

} // namespace light_hash_add_items
} // namespace Mips2C
// add light_hash_add_items::link to the link callback table for the object file.
// FWD DEC:
...
// add light_hash_count_items::link to the link callback table for the object file.
// FWD DEC:
