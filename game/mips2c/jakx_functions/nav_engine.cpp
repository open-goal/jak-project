...
block_1:
  ...
  bc = c->sgpr64(s7) == c->sgpr64(t1);              // beq s7, t1, L30
...
block_4:
  ...
  bc = c->sgpr64(a3) != 0;                          // bne a3, r0, L29
  // nop                                            // sll r0, r0, 0
...
void link() {
  gLinkedFunctionTable.reg("(method 21 nav-engine)", execute, 256);
}
...
block_1:
  ...
  c->daddiu(t0, a3, 112);                           // daddiu t0, a3, 112
  c->lwu(t1, 8, a3);                                // lwu t1, 8(a3)
  bc = c->sgpr64(s7) == c->sgpr64(t1);              // beq s7, t1, L35
  c->mov64(t1, s7);                                 // or t1, s7, r0
  ...
  bc = c->sgpr64(t1) == 0;                          // beq t1, r0, L34
  c->mov64(t1, s7);                                 // or t1, s7, r0
...
block_6:
  c->lb(a3, 14, a1);                                // lb a3, 14(a1)
  c->slt(a3, a2, a3);                               // slt a3, a2, a3
  bc = c->sgpr64(a3) != 0;                          // bne a3, r0, L33
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
...
void link() {
  gLinkedFunctionTable.reg("(method 20 nav-engine)", execute, 256);
}
...
block_1:
  ...
  // nop                                            // sll r0, r0, 0
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(a3) != 0;                          // bne a3, r0, L44
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
...
void link() {
  gLinkedFunctionTable.reg("(method 18 nav-engine)", execute, 256);
}
...
block_1:
  ...
  // nop                                            // sll r0, r0, 0
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(a3) != 0;                          // bne a3, r0, L46
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
...
void link() {
  gLinkedFunctionTable.reg("(method 17 nav-engine)", execute, 256);
}
...
void link() {
  gLinkedFunctionTable.reg("nav-state-patch-pointers", execute, 256);
}
...
block_1:
  ...
  // nop                                            // sll r0, r0, 0
  // nop                                            // sll r0, r0, 0
  bc = c->sgpr64(a3) != 0;                          // bne a3, r0, L50
  // nop                                            // sll r0, r0, 0
  if (bc) {goto block_1;}                           // branch non-likely
...
void link() {
  gLinkedFunctionTable.reg("nav-dma-send-from-spr-no-flush", execute, 256);
}
...
void link() {
  gLinkedFunctionTable.reg("nav-dma-send-to-spr-no-flush", execute, 256);
}
