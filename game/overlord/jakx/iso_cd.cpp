...
void jakx_overlord_init_globals_iso_cd() {
  g_pReadInfo = nullptr;
...
CISOCDFile::CISOCDFile(const jakx::ISOFileDef* filedef, s32 process_data_semaphore)
 ...
 */
void CISOCDFile::ReadPagesCallback(jakx::Block* block, int error) {
  if (error == 0) {
 ...
 */
ISOFileDef* CISOCDFileSystem::FindIN(const jakx::ISOName* name) {
 ...
 */
int CISOCDFileSystem::GetLength(const jakx::ISOFileDef* file) {
  // return file->length;
  lg::info("getlength");
 ...
 */
CBaseFile* CISOCDFileSystem::Open(const jakx::ISOFileDef* file_def,
                                  int sector_offset,
                                  int file_kind) {
 ...
 */
CBaseFile* CISOCDFileSystem::OpenWAD(const jakx::ISOFileDef* file_def, int page_offset) {
  auto* file = AllocateFile(file_def);
  ASSERT(file);
 ...
 */
CISOCDFile* CISOCDFileSystem::AllocateFile(const jakx::ISOFileDef* file_def) {
