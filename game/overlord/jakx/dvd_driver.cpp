...
void jakx_overlord_init_globals_dvd_driver() {
...
CMsg::CMsg(jakx::CMsg::MsgKind msg) : m_msg(msg) {
  m_ret = 1;
...
// CMsgReadRaw::CMsgReadRaw(jakx::BlockParams* params) : CMsg(CMsg::MsgKind::READ_RAW) {
...
CMsgCancelRead::CMsgCancelRead(jakx::CDescriptor* desc) : CMsg(CMsg::MsgKind::CANCEL_READ) {
  m_desc = desc;
}
...
void CDvdDriver::CancelRead(jakx::CDescriptor* desc) {
  if (GetThreadId() == g_nDvdDriverThread) {
...
s32 CDvdDriver::ValidateBlockParams(jakx::BlockParams* params, int num_params) {
  if (!params) {
 ...
 */
void CDvdDriver::read_from_file(const jakx::Block* block) {
