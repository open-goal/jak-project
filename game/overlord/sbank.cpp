...
void InitBanks() {
  ...
  if (g_game_version == GameVersion::Jak2) {
    strncpy(gBanks[0]->name, "common", sizeof(SoundBank::name));
    gBanks[0]->spu_loc = 0x20000;
    gBanks[0]->spu_size = 0xAFCC0;

    strncpy(gBanks[1]->name, "gun", sizeof(SoundBank::name));
    gBanks[1]->spu_loc = 0x131740;
    gBanks[1]->spu_size = 0;

    strncpy(gBanks[2]->name, "board", sizeof(SoundBank::name));
    gBanks[2]->spu_loc = 0x131740;
    gBanks[2]->spu_size = 0;

    strncpy(gBanks[3]->name, "level0", sizeof(SoundBank::name));
    gBanks[3]->spu_loc = 0x131740;
    gBanks[3]->spu_size = 0x42800;

    strncpy(gBanks[4]->name, "level1", sizeof(SoundBank::name));
    gBanks[4]->spu_loc = 0x173f40;
    gBanks[4]->spu_size = 0x42800;

    strncpy(gBanks[5]->name, "level2", sizeof(SoundBank::name));
...
s32 LookupSoundIndex(const char* name, SoundBank** bank_out) {
  for (auto bank : gBanks) {
    ...
    for (int i = 0; i < (int)bank->sound_count; i++) {
      if (memcmp(bank->sound[i].name, name, sizeof(SoundRecord::name)) == 0) {
        *bank_out = bank;
        return i;
      }
    }
...
SoundBank* LookupBank(const char* name) {
  ...
  while (true) {
    ...
    if (g_game_version == GameVersion::Jak1) {
      if ((memcmp(bank->name, name, sizeof(bank->name)) == 0)) {
        return bank;
      }
    } else {
      if ((memcmp(bank->name, name, sizeof(bank->name)) == 0) && bank->in_use) {
        return bank;
      }
    }
    idx--;
