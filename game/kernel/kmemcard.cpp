...
void pc_game_save_synch() {
  ...
  mc_print("synchronous save file open took {:.2f}ms", mc_timer.getMs());
  if (fd) {
    // cb_openedsave //
  ...
  mc_print("[MC] synchronous save took {:.2f}ms", mc_timer.getMs());
}
...
void pc_game_load_synch() {
  ...
  mc_print("synchronous load took {:.2f}ms", mc_timer.getMs());
}
