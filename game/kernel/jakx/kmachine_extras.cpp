...
AutoSplitterBlock g_auto_splitter_block_jakx;
...
void update_discord_rpc(u32 discord_info) {
  if (gDiscordRpcEnabled) {
    ...
    if (info) {
      ...
      // convert encodings
      std::string status = get_font_bank(GameTextVersion::JAKX)
...
static std::string unpack_vag_name_jakx(u64 compressed) {
  ...
  if (alloced_heap.offset) {
    *alloced_heap = g_VagDir.num_entries;
    ...
    for (size_t i = 0; i < g_VagDir.num_entries; ++i) {
      ...
      auto name = unpack_vag_name_jakx(packed);
...
void init_autosplit_struct() {
  g_auto_splitter_block_jakx.pointer_to_symbol =
      (u64)g_ee_main_mem + (u64)intern_from_c(-1, 0, "*autosplit-info-jakx*")->value();
}
  ...
  if (external_speedrun_time_cache.find(speedrun_id) != external_speedrun_time_cache.end()) {
    ...
    if (index < (int)runs.size()) {
      ...
      std::string converted =
          get_font_bank(GameTextVersion::JAKX)->convert_utf8_to_game(run_info.first);
      strcpy(Ptr<String>(name_dest_ptr).c()->data(), converted.c_str());
      *(Ptr<float>(time_dest_ptr).c()) = run_info.second;
    } else {
      std::string converted = get_font_bank(GameTextVersion::JAKX)->convert_utf8_to_game("");
      strcpy(Ptr<String>(name_dest_ptr).c()->data(), converted.c_str());
