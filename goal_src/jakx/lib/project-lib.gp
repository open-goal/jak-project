...
(defmacro goal-src (src-file &rest deps)
  ...
  `(let ((output-file ,(gc-file->o-file src-file)))
    ...
    (defstep :in ,(string-append "goal_src/jakx/" src-file)
     ;; use goal compiler
     :tool 'goalc
...
(defun make-src-sequence-elt (current previous prefix)
  ...
  `(let ((output-file ,(gc-file->o-file current)))
    (set! *all-gc* (cons output-file *all-gc*))
    (defstep :in ,(string-append "goal_src/jakx/" prefix current)
     :tool 'goalc
     :out (list output-file)
...
(defun cgo (output-name desc-file-name)
  "Add a CGO with the given output name (in $OUT/iso) and input name (in goal_src/jakx/dgos)"
  (let ((out-name (string-append "$OUT/iso/" output-name)))
    (defstep :in (string-append "goal_src/jakx/dgos/" desc-file-name)
      :tool 'dgo
      :out `(,out-name)
      )
...
(defmacro cgo-file (dgo-file-name deps &key (bsp-file-name #f))
  ...
  (let ((dgo-data (car (read-data-file (string-append "goal_src/jakx/dgos/" dgo-file-name)))))
...
(defun custom-level-cgo (output-name desc-file-name)
  "Add a CGO with the given output name (in $OUT/iso) and input name (in custom_assets/jakx/levels/)"
  (let ((out-name (string-append "$OUT/iso/" output-name)))
    (defstep :in (string-append "custom_assets/jakx/levels/" desc-file-name)
      :tool 'dgo
      :out `(,out-name)
      )
    (set! *all-cgos* (cons out-name *all-cgos*))
...
(defmacro build-custom-level (name &key (force-run #f) &key (gen-fr3 #t))
  (let* ((path (string-append "custom_assets/jakx/levels/" name "/" name ".jsonc")))
...
(defmacro build-actor (name &key (gen-mesh #f) &key (force-run #f) &key (texture-bucket 0))
  (let* ((path (string-append "custom_assets/jakx/models/custom_levels/" name ".glb")))
