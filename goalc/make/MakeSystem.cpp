...
MakeSystem::MakeSystem() {
  ...
  add_tool(std::make_shared<DgoTool>());
}
...
void MakeSystem::load_project_file(const std::string& file_path) {
  ...
  fmt::print("Loaded project file for {} targets successfully in {:.3f}s.\n",
             m_output_to_step.size(), timer.getSeconds());
}

goos::Object MakeSystem::handle_defstep(const goos::Object& form,
                                        goos::Arguments& args,
                                        const std::shared_ptr<goos::EnvironmentObject>&) {
  va_check(form, args, {},
...
goos::Object MakeSystem::handle_basename(const goos::Object& form,
  ...
  return goos::StringObject::make_new(input.filename());
}
...
goos::Object MakeSystem::handle_stem(const goos::Object& form,
  ...
  return goos::StringObject::make_new(input.stem());
}
...
bool MakeSystem::make(const std::string& target, bool force) {
  ...
  for (auto& to_make : deps) {
    ...
    if (!tool->run({rule->input, rule->deps, rule->outputs})) {
