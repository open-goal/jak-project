...
Compiler::ConstPropResult Compiler::constant_propagation_dispatch(const goos::Object& code,
  ...
  switch (expanded.type) {
    ...
    case goos::ObjectType::SYMBOL: {
      ...
      while (mlet_env) {
        auto mlkv = mlet_env->macros.find(expanded.as_symbol().name_ptr);
      ...
      const auto& global_constant = m_global_constants.find(expanded.as_symbol().name_ptr);
      const auto& existing_symbol = m_symbol_types.find(expanded.as_symbol().name_ptr);
