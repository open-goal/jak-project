...
TEST(EmitterIntegerMath, add_gpr64_imm8s) {
  ...
  tester.clear();
  tester.emit(IGen::add_gpr64_imm8s(tester.generator(), RSP, 12));
...
TEST(EmitterIntegerMath, add_gpr64_imm32s) {
  ...
  tester.clear();
  tester.emit(IGen::add_gpr64_imm32s(tester.generator(), RSP, 12));
...
TEST(EmitterIntegerMath, sub_gpr64_imm8s) {
  ...
  tester.clear();
  tester.emit(IGen::sub_gpr64_imm8s(tester.generator(), RSP, 12));
...
TEST(EmitterIntegerMath, sub_gpr64_imm32s) {
  ...
  tester.clear();
  tester.emit(IGen::sub_gpr64_imm32s(tester.generator(), RSP, 12));
...
TEST(EmitterLoadsAndStores, store8_gpr64_gpr64_plus_gpr64) {
  ...
  tester.clear();
  tester.emit(IGen::store8_gpr64_gpr64_plus_gpr64(tester.generator(), RAX, RCX, RDX));
...
TEST(EmitterLoadsAndStores, store8_gpr64_gpr64_plus_gpr64_plus_s8) {
  ...
  tester.clear();
  tester.emit(IGen::store8_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, RDX, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "88 54 01 0c");

  auto instr = IGen::store8_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s8*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, store8_gpr64_gpr64_plus_gpr64_plus_s32) {
  ...
  tester.clear();
  tester.emit(IGen::store8_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, RDX, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "88 94 01 0c 00 00 00");

  auto instr = IGen::store8_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s32*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, store16_gpr64_gpr64_plus_gpr64) {
  ...
  tester.clear();
  tester.emit(IGen::store16_gpr64_gpr64_plus_gpr64(tester.generator(), RCX, RAX, R8));
...
TEST(EmitterLoadsAndStores, store16_gpr64_gpr64_plus_gpr64_plus_s8) {
  ...
  tester.clear();
  tester.emit(IGen::store16_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, R8, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "66 44 89 44 01 0c");

  auto instr = IGen::store16_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s8*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, store16_gpr64_gpr64_plus_gpr64_plus_s32) {
  ...
  tester.clear();
  tester.emit(IGen::store16_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, R8, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "66 44 89 84 01 0c 00 00 00");

  auto instr = IGen::store16_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s8*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, store32_gpr64_gpr64_plus_gpr64) {
  ...
  tester.clear();
  tester.emit(IGen::store32_gpr64_gpr64_plus_gpr64(tester.generator(), RCX, RAX, R8));
...
TEST(EmitterLoadsAndStores, store32_gpr64_gpr64_plus_gpr64_plus_s8) {
  ...
  tester.clear();
  tester.emit(IGen::store32_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, R8, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "44 89 44 01 0c");

  auto instr = IGen::store32_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s8*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, store32_gpr64_gpr64_plus_gpr64_plus_s32) {
  ...
  tester.clear();
  tester.emit(IGen::store32_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, R8, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "44 89 84 01 0c 00 00 00");

  auto instr = IGen::store32_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s8*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, store64_gpr64_gpr64_plus_gpr64) {
  ...
  tester.clear();
  tester.emit(IGen::store64_gpr64_gpr64_plus_gpr64(tester.generator(), RCX, RAX, R8));
...
TEST(EmitterLoadsAndStores, store64_gpr64_gpr64_plus_gpr64_plus_s8) {
  ...
  tester.clear();
  tester.emit(IGen::store64_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, R8, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "4c 89 44 01 0c");

  auto instr = IGen::store64_gpr64_gpr64_plus_gpr64_plus_s8(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s8*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, store64_gpr64_gpr64_plus_gpr64_plus_s32) {
  ...
  tester.clear();
  tester.emit(IGen::store64_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, R8, 12));
  EXPECT_EQ(tester.dump_to_hex_string(), "4c 89 84 01 0c 00 00 00");

  auto instr = IGen::store64_gpr64_gpr64_plus_gpr64_plus_s32(tester.generator(), RAX, RCX, RDX, -3);
  u8 buff[256];
  instr.emit(buff);
  EXPECT_EQ(*(s8*)(buff + instr.offset_of_disp()), -3);
...
TEST(EmitterLoadsAndStores, static_addr) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    auto lea_instr = IGen::static_addr(tester.generator(), i, INT32_MAX);
  ...
  for (int i = 0; i < 16; i++) {
    ...
    for (int j = 0; j < 16; j++) {
      ...
      for (int k = 0; k < 16; k++) {
        ...
        tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + k, i));
...
TEST(EmitterXmm32, static_load_xmm32) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    auto load_instr = IGen::static_load_f32(tester.generator(), XMM0 + i, INT32_MAX);

    tester.emit(load_instr);
    tester.emit(IGen::movd_gpr32_f32(tester.generator(), RAX, XMM0 + i));
    tester.emit_pop_all_gprs(true);
    tester.emit_pop_all_simd();
    tester.emit_return();
...
TEST(EmitterXmm32, static_store_xmm32) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    tester.emit_push_all_simd();
    tester.emit_push_all_gprs(true);
    tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + i, tester.get_c_abi_arg_reg(0)));

    auto loc_of_store = tester.size();
    auto store_instr = IGen::static_store_f32(tester.generator(), XMM0 + i, INT32_MAX);
    ...
    for (auto g : vals) {
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          ...
          tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + i, RAX));
          memcpy(&val, &g, sizeof(float));
          tester.emit(IGen::mov_gpr64_u64(tester.generator(), RAX, val));
          tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + j, RAX));
          tester.emit(IGen::mul_f32_f32(tester.generator(), XMM0 + j, XMM0 + i));
          tester.emit(IGen::movd_gpr32_f32(tester.generator(), RAX, XMM0 + j));
          tester.emit_pop_all_gprs(true);
          tester.emit_pop_all_simd();
          tester.emit_return();
    ...
    for (auto g : vals) {
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          ...
          tester.emit(IGen::mov_gpr64_u64(tester.generator(), RAX, val));
          tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + j, RAX));
          tester.emit(IGen::div_f32_f32(tester.generator(), XMM0 + j, XMM0 + i));
          tester.emit(IGen::movd_gpr32_f32(tester.generator(), RAX, XMM0 + j));
          tester.emit_pop_all_gprs(true);
          tester.emit_pop_all_simd();
          tester.emit_return();
    ...
    for (auto g : vals) {
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          ...
          tester.emit(IGen::mov_gpr64_u64(tester.generator(), RAX, val));
          tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + j, RAX));
          tester.emit(IGen::add_f32_f32(tester.generator(), XMM0 + j, XMM0 + i));
          tester.emit(IGen::movd_gpr32_f32(tester.generator(), RAX, XMM0 + j));
          tester.emit_pop_all_gprs(true);
          tester.emit_pop_all_simd();
          tester.emit_return();
    ...
    for (auto g : vals) {
      for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
          ...
          tester.emit(IGen::mov_gpr64_u64(tester.generator(), RAX, val));
          tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + j, RAX));
          tester.emit(IGen::sub_f32_f32(tester.generator(), XMM0 + j, XMM0 + i));
          tester.emit(IGen::movd_gpr32_f32(tester.generator(), RAX, XMM0 + j));
          tester.emit_pop_all_gprs(true);
          tester.emit_pop_all_simd();
          tester.emit_return();
  ...
  for (auto g : vals) {
    for (int i = 0; i < 16; i++) {
      for (int j = 0; j < 16; j++) {
        ...
        tester.emit(IGen::mov_gpr64_u64(tester.generator(), RAX, val));
        tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + i, RAX));
        tester.emit(IGen::f32_to_int32(tester.generator(), j, XMM0 + i));
  ...
  for (auto g : vals) {
    for (int i = 0; i < 16; i++) {
      for (int j = 0; j < 16; j++) {
        ...
        tester.emit(IGen::int32_to_f32(tester.generator(), XMM0 + i, j));
        tester.emit(IGen::movd_gpr32_f32(tester.generator(), RAX, XMM0 + i));
        tester.emit_pop_all_gprs(true);
        tester.emit_pop_all_simd();
        tester.emit_return();
      ...
      for (int r2 = 0; r2 < 16; r2++) {
        ...
        for (int r3 = 0; r3 < 16; r3++) {
          for (int r4 = 0; r4 < 16; r4++) {
            ...
            tester.emit(IGen::mov_gpr64_u32(tester.generator(), r1, constant));
            // move gpr to xmm
            tester.emit(IGen::movd_f32_gpr32(tester.generator(), XMM0 + r3, r1));
            // move xmm to xmm
            tester.emit(IGen::mov_f32_f32(tester.generator(), XMM0 + r4, XMM0 + r3));
            // move xmm to gpr
            tester.emit(IGen::movd_gpr32_f32(tester.generator(), r2, XMM0 + r4));
            // return!
            tester.emit(IGen::mov_gpr64_gpr64(tester.generator(), RAX, r2));
            tester.emit_pop_all_gprs(true);
            tester.emit_pop_all_simd();
...
TEST(Emitter, LEA) {
  ...
  tester.emit(IGen::lea_reg_plus_off(tester.generator(), RDI, R12, -3));
  tester.emit(IGen::lea_reg_plus_off(tester.generator(), R13, RSP, -3));
  tester.emit(IGen::lea_reg_plus_off(tester.generator(), R13, R12, -3));
  tester.emit(IGen::lea_reg_plus_off(tester.generator(), RDI, RSP, -300));
  tester.emit(IGen::lea_reg_plus_off(tester.generator(), RDI, R12, -300));
  tester.emit(IGen::lea_reg_plus_off(tester.generator(), R13, RSP, -300));
  tester.emit(IGen::lea_reg_plus_off(tester.generator(), R13, R12, -300));
  EXPECT_EQ(tester.dump_to_hex_string(true),
...
TEST(EmitterXMM, StackLoadFull32) {
  ...
  tester.emit(IGen::load128_simd128_gpr64_s32(tester.generator(), XMM0 + 13, RSP, -1234));
...
TEST(EmitterXMM, StackLoadFull8) {
  ...
  tester.emit(IGen::load128_simd128_gpr64_s8(tester.generator(), XMM0 + 13, RSP, -12));
...
TEST(EmitterXMM, StackStoreFull32) {
  ...
  tester.emit(IGen::store128_gpr64_simd128_s32(tester.generator(), RSP, XMM0 + 13, -1234));
...
TEST(EmitterXMM, StackStoreFull8) {
  ...
  tester.emit(IGen::store128_gpr64_simd128_s8(tester.generator(), RSP, XMM0 + 13, -12));
...
TEST(EmitterXMM, SqrtS) {
  ...
  tester.emit(IGen::sqrt_f32(tester.generator(), XMM0 + 11, XMM0 + 2));
  tester.emit(IGen::sqrt_f32(tester.generator(), XMM0 + 1, XMM0 + 12));
  tester.emit(IGen::sqrt_f32(tester.generator(), XMM0 + 11, XMM0 + 12));
