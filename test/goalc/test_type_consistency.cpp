...
TEST(Jak2TypeConsistency, MANUAL_TEST_TypeConsistencyWithBuildFirst) {
  Compiler compiler(GameVersion::Jak2, emitter::kNativeInstructionSet);
  compiler.enable_throw_on_redefines();
  add_common_expected_type_mismatches(compiler);
...
TEST(Jak3TypeConsistency, MANUAL_TEST_TypeConsistencyWithBuildFirst) {
  Compiler compiler(GameVersion::Jak3, emitter::kNativeInstructionSet);
  compiler.enable_throw_on_redefines();
  add_common_expected_type_mismatches(compiler);
...
TEST(JakXTypeConsistency, TypeConsistencyWithBuildFirst) {
  Compiler compiler(GameVersion::JakX, emitter::kNativeInstructionSet);
  compiler.enable_throw_on_redefines();
  add_common_expected_type_mismatches(compiler);
