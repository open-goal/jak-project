...
Interpreter::Interpreter(const std::string& username) {
  m_true_obj = intern("#t");
  m_false_obj = intern("#f");
  m_true_sym = m_true_obj.as_symbol().name_ptr;
  m_false_sym = m_false_obj.as_symbol().name_ptr;
  // Interpreter startup:
  // create the GOOS global environment
  global_environment = EnvironmentObject::make_new("global");

  // create the environment which should be visible from GOAL
...
Interpreter::~Interpreter() {
  ...
  // don't do this.
  global_environment.as_env()->vars_by_st_string.clear();
  goal_env.as_env()->vars_by_st_string.clear();
}
...
void Interpreter::define_var_in_env(Object& env, const Object& var, const std::string& name) {
  env.as_env()->vars_by_st_string.set(intern_ptr(name), var);
}
...
const char* Interpreter::intern_ptr(const std::string& name) {
...
void Interpreter::set_global_variable_to_symbol(const std::string& name, const std::string& value) {
  define_var_in_env(global_environment, Object::make_symbol(&reader.symbolTable, value.c_str()),
                    name);
}
...
Arguments Interpreter::get_args(const Object& form, const Object& rest, const ArgumentSpec& spec) {
  ...
  while (!current->is_empty_list()) {
    ...
    if (arg.is_symbol() && arg.as_symbol().name_ptr[0] == ':') {
      const char* key_name = arg.as_symbol().name_ptr + 1;
...
bool try_symbol_lookup(const Object& sym,
  ...
  for (;;) {
    const auto& kv = search_env->vars_by_st_string.lookup(sym.as_symbol().name_ptr);
    if (kv) {
      *dest = *kv;
      return true;
    }
...
void Interpreter::set_args_in_env(const Object& form,
  ...
  for (size_t i = 0; i < arg_spec.unnamed.size(); i++) {
    env->vars_by_st_string.set(intern_ptr(arg_spec.unnamed.at(i).c_str()), args.unnamed.at(i));
  }

  // named args
  for (const auto& kv : arg_spec.named) {
    env->vars_by_st_string.set(intern_ptr(kv.first), args.named.at(kv.first));
  }

  // rest args
  if (!arg_spec.rest.empty()) {
    // will correctly handle the '() case
    env->vars_by_st_string.set(intern_ptr(arg_spec.rest), build_list(args.rest));
  } else {
    if (!args.rest.empty()) {
      throw_eval_error(form, "got too many arguments");
    }
...
Object Interpreter::eval_define(const Object& form,
  ...
  define_env->vars_by_st_string.set(args.unnamed[0].as_symbol().name_ptr, value);
  return value;
}
...
Object Interpreter::eval_set(const Object& form,
  ...
  for (;;) {
    auto kv = search_env->vars_by_st_string.lookup(to_define.as_symbol().name_ptr);
    if (kv) {
      search_env->vars_by_st_string.set(to_define.as_symbol().name_ptr, to_set);
      return to_set;
    }
...
Object Interpreter::eval_equals(const Object& form,
  ...
  (void)env;
  vararg_check(form, args, {{}, {}}, {});
  return args.unnamed[0] == args.unnamed[1] ? m_true_obj : m_false_obj;
}
...
Object Interpreter::eval_numequals(const Object& form,
  ...
  return result ? m_true_obj : m_false_obj;
}
...
Object Interpreter::num_lt(const Object& form,
  ...
  return (a < b) ? m_true_obj : m_false_obj;
}
...
Object Interpreter::eval_null(const Object& form,
  ...
  (void)env;
  vararg_check(form, args, {{}}, {});
  return args.unnamed[0].is_empty_list() ? m_true_obj : m_false_obj;
}
...
Object Interpreter::eval_hash_table_try_ref(const Object& form,
  ...
  if (it == table->data.end()) {
    // not in table
    return PairObject::make_new(m_false_obj, Object::make_empty_list());
  } else {
    return PairObject::make_new(m_true_obj, it->second);
  }
}
}  // namespace goos
